  ...
  constructor() {
    ...
    this.logText = {
      ERROR: (message: string, err: Error, extra: any) =>
        `[\x1b[1m\x1b[31mERROR\x1b[0m]: ${message} Extra: ${JSON.stringify(
          extra
        )}; Stack: (${err.stack})`,
      WARN: (message: string, err: Error, extra: any) =>
        `[\x1b[33mWARN\x1b[0m]: ${message} Extra: ${JSON.stringify(
          extra
        )}; Stack: (${/\/.+?:\d*:\d*/.exec(err.stack as string)![0]})`,
      DEBUG: (message: string, err: Error, extra: any) =>
        `[\x1b[1m\x1b[35mDEBUG\x1b[0m]: ${message} Extra: ${JSON.stringify(
          extra
        )}; Stack: (${/\/.+?:\d*:\d*/.exec(err.stack as string)![0]})`,
      INFO: (message: string, err: Error, extra: any) =>
        `[\x1b[36mINFO\x1b[0m]: ${message} Extra: ${JSON.stringify(
          extra
        )}; Stack: (${/\/.+?:\d*:\d*/.exec(err.stack as string)![0]})`,
    };
  }
